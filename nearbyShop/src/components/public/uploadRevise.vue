<template>
  <div class="upload-container" :style="{width:this.conWidth}">
    <div class="divs">
      <p class="p" :style="{width: this.width + 'px'}">{{title}}</p>
      <div class="add-image mt10">
        <div class="demo-upload-list" v-for="(item,index) in uploadList">
          <template v-if="item.status === 'finished'">
            <img :src="item.url">
            <div class="demo-upload-list-cover">
              <Icon type="ios-arrow-back" @click="indexBack(item,index)" v-if="uploadList.length > 1"/>
              <Icon type="ios-eye-outline" @click.native="handleView(item.url)"></Icon>
              <Icon type="ios-trash-outline" @click.native="handleRemove(item)"></Icon>
              <Icon type="ios-arrow-forward" @click="indexForward(item,index)" v-if="uploadList.length > 1"/>
            </div>
          </template>
          <template v-else>
            <Progress v-if="item.showProgress" :percent="item.percentage" hide-info></Progress>
          </template>
        </div>
        <Upload :action="uploadUrl"
                ref="upload"
                :show-upload-list="false"
                :default-file-list="defaultList"
                :data="params"
                :on-success="handleSuccess"
                :format="['jpg','jpeg','png','bmp']"
                :on-format-error="handleFormatError"
                :before-upload="handleBeforeUpload"
                multiple
                type="drag"
                style="display: inline-block;width:130px;">
          <div style="width: 130px;height:130px;line-height: 130px;">
            <Icon type="ios-camera" size="40"></Icon>
          </div>
        </Upload>
        <modal title="查看照片" v-model="visible">
          <img :src="bigUrl" style="width: 100%">
          <div slot="footer"></div>
        </modal>
      </div>
    </div>
    <Modal
      title="提示"
      v-model="tip"
      :mask-closable = "false"
    >
      <p ref="tip"></p>
      <div slot="footer">
        <Button type="info"  @click="del">确定</Button>
      </div>
    </Modal>
  </div>
</template>

<script>
  import axios from "../../axios/axios";
  export default {
    name: "reviseUpload",
    props:{
      defaultList:{},  //默认图片即已上传图片
      uploadUrl:{}, //照片上传地址
      title:{},     //照片标题
      length:{},    //允许上传几张照片
      width:{},     //标题宽度 px
      conWidth:{},  //组件宽度 百分比
      listValue:{},  //监测是否出现
      params:{}   //额外参数
    },
    data () {
      return {
        uploadList:[],
        visible:false,
        bigUrl:'',
        tip:false
      }
    },
    mounted () {
      this.$refs.upload.fileList = this.defaultList;
      this.uploadList = this.$refs.upload.fileList;
    },
    watch:{  //通过监听listValue值得变化来加载defaultList
      listValue(value){
        if (value) {
          this.$refs.upload.fileList = this.defaultList;
          this.uploadList = this.$refs.upload.fileList;
        } else {
          this.uploadList = [];
        }
      }
    },
    methods:{
      handleView:function (url) {
        this.visible = true;
        this.bigUrl = url;
      },
      handleRemove:function (file) {
        const fileList = this.$refs.upload.fileList;
        this.$refs.upload.fileList.splice(fileList.indexOf(file), 1);
        this.uploadList = this.$refs.upload.fileList;
        this.$emit('remove-callback',file,this.uploadList);
      },
      handleSuccess:function (res,file,fileList) {
        if (res.code === 200) {
          file.url = res.result.filePath;
          this.uploadList = fileList;
          this.$emit('success-callback',file,fileList);
        } else {
          this.tip = true;
          this.$refs.tip.innerHTML = '图片上传失败！';
        }
      },
      handleFormatError:function () {
        this.$Message.warning('请选择格式为“jpg，jpeg，png，bmp”格式的图片！');
      },
      handleBeforeUpload:function (file) {
        const check = this.uploadList.length < this.length;
        const len = file.name.length <= 50;
        if (!len) {
          this.$Message.warning('图片名过长！');
          return len;
        }
        if (!check) {
          this.$Message.warning('只能上传'+this.length+'张图片！');
          return check;
        }
      },
      indexBack:function (file,index) {
        if (index === 0) {
          return;
        }
        let arr = this.uploadList;
        arr.splice(index-1,0,file);
        arr.splice(index+1,1);
        this.uploadList = arr;
        this.$emit('position-callback',this.uploadList);
      },
      indexForward:function (file,index) {
        if (index === this.uploadList.length-1){
          return;
        }
        let arr = this.uploadList;
        arr.splice(index+2,0,file);
        arr.splice(index,1);
        this.uploadList = arr;
        this.$emit('position-callback',this.uploadList);
      },
      del:function () {
        this.tip = false;
      },
      clearUpload:function () {
        this.$refs.upload.clearFiles();
        this.uploadList = this.$refs.upload.fileList;
      }
    }
  }
</script>

<style lang="less" scoped>
  .divs{
    display: flex;
    flex-direction: row;
    justify-content: flex-start;
  }
  .add-image{
    display: flex;
    flex-direction: row;
    justify-content: flex-start;
    flex-wrap: wrap;
  }
  .p{
    height: 90px;
    line-height: 90px;
    text-align: right;
    padding: 10px 12px 10px 0;
  }
  .demo-upload-list{
    display: inline-block;
    width: 130px;
    height: 130px;
    text-align: center;
    line-height: 130px;
    border: 1px solid transparent;
    border-radius: 4px;
    overflow: hidden;
    background: #fff;
    position: relative;
    box-shadow: 0 1px 1px rgba(0,0,0,.2);
    margin-right: 4px;
  }
  .demo-upload-list img{
    width: 100%;
    height: 100%;
  }
  .demo-upload-list-cover{
    display: none;
    position: absolute;
    top: 0;
    bottom: 0;
    left: 0;
    right: 0;
    background: rgba(0,0,0,.6);
  }
  .demo-upload-list:hover .demo-upload-list-cover{
    display: block;
  }
  .demo-upload-list-cover i{
    color: #fff;
    font-size: 20px;
    cursor: pointer;
    margin: 0 2px;
  }
</style>

